...
// The unified runtime resolves centrally, so no caller can forget to. The
// implementation moved from the old monolithic src/omnivoice.cpp into the
// pipeline during the OmniVoice runtime consolidation; assert that new funnel
// directly rather than retaining a stale source-path contract.
TEST_CASE("omnivoice lang: the runtime resolves before building the prompt", "[unit][omnivoice]") {
    const std::string src = read_file("src/omnivoice/pipeline-tts.cpp");
    REQUIRE(contains(src, "core/omnivoice_lang.h"));
    REQUIRE(contains(src, "core_omnivoice_lang::resolve("));
    REQUIRE(contains(src, "pipeline_tts_resolve_language"));
    ...
    REQUIRE(contains(src, "CRISPASR_OMNIVOICE_AUTO_LANG"));
}
...
TEST_CASE("omnivoice lang: the CLI adapter applies language PER CALL", "[unit][omnivoice]") {
    ...
    // The unified ABI receives a fresh ov_tts_params on every request; target
    // language is selected here rather than stored once during init().
    REQUIRE(contains(body, "fill_tts_params(text, params, &tp)"));
    REQUIRE(contains(body, "target_lang"));
    REQUIRE(contains(body, "tp->lang"));
    ...
    REQUIRE(contains(body, "tp->mg_seed"));
}

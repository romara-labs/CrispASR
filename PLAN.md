...
| input | single-pass (CANARY_STREAM_THRESHOLD_S=120) | first cut (`7177c931`) | per-chunk (`63fdbe46`) |
|---|---|---|---|
| JFK 11 s, default path | "...for you, ask what you can do for your country." ✓ | (single-pass, unchanged) | (single-pass, unchanged) |
| JFK 11 s, forced streamed | n/a | "...for you." (truncated) | "...for you, Country can do for you. Ask what you can do for your country." (boundary-overlap duplication) |
| yt_60s.wav (Japanese audio, -l en) | empty output (encoder over 60 s collapses) | "I'm not going to go to the next video." (one short hallucination) | romanized Japanese for in-distribution chunks + degenerate "yeah yeah" loops for OOD chunks (60 s → 11 s wall = 5.3× RT) |

The per-chunk path is strictly more recoverable. JFK forced-streamed showing boundary duplication is the polish-worthy artifact — `core/lcs_merge::merge_overlapping_hypotheses` is wired in for voxtral (PLAN #114 P2) and is the natural next step here. The 60 s OOD-audio "yeah" loop is the AED decoder hitting a no-repetition-penalty failure mode; same shape as the funasr `!`-loop fix in PLAN #125 P1, applies symmetrically here as a follow-up.

**Status.** Default `CANARY_STREAM_THRESHOLD_S=30` retained; flipping to `0` (always streamed) waits on the dedup polish landing. Functionally, the long-audio path is correct now — the regression vs single-pass is bounded duplication, not lost content.

...
| **HIGH** | [#57 Commercial-friendly TTS expansion](#57-commercial-friendly-tts-backend-expansion) | Phased | Phases 1-3 DONE; Turbo WORKING; F0 wired in; native voice cloning shipped → HISTORY §82; **#83 production fix LANDED 2026-05-24 → HISTORY 2026-05-24 + LEARNINGS Round 9** — S3Gen UNet weight residency split (`s3.fd.*` on CPU, encoder/vocoder GPU): M1 Metal cos_min 0.940→**0.999980** in diff harness, intelligible audio at all T; comparable wall-time to pure CPU on M1. Q8_0×F32 bit-match Metal kernel committed (commit `752baecf`, upstream-PR-quality, drafted as PR 09). 3 upstream PR drafts in `tools/upstream-prs/09-11` covering Metal Q8_0 kernel + ggml-alloc drift bug report + scheduler NaN-at-large-T bug report. Linux CPU smoke validated on VPS. **FIXED** 2026-05-24 (R9 follow-up #4): the bug was the sched-internal CPU→GPU copy of the three runtime inputs (`unet_input`, `time_emb`, `mask`). Without an explicit backend pin, sched places them on CPU and the auto-generated copy node feeds stale data to the first Metal kernel of every CFM step — `after_im2col` cos=-0.08 from the very first kernel; downstream propagates to NaN under certain `set_output` layouts. Fix: pin those three inputs to `c->backend` in `cfm_euler_solve::run_denoiser` when `c->unet_on_gpu` is true. M1 Metal diff `s3gen_mel cos_min 0.940 → 0.999976` (parity with production CPU residency), baseline `vocoder mel rms 13.938 → 5.143`, 2-mark trigger NaN → finite. Production CPU-residency path unchanged. Also open: Kartoffelbox_Turbo DE |

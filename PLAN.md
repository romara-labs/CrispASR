...
**Status:** Phase 1 merged (c4da639c). Phases 2-4 in progress on `worktree-feat+tts-watermark-metadata`.
...
disclosure and consent. This plan covers all four layers.

### Phase 1: Watermark + file metadata ✅ MERGED
...
### Phase 2: Consent gate for voice cloning

**What:** Require explicit attestation before voice cloning. Log it.

**Files to modify:**
- `examples/cli/whisper_params.h` — add `bool tts_voice_clone_consent = false;`
  and `std::string tts_consent_attestation;`
- `examples/cli/cli.cpp` — parse `--i-have-rights` flag with help text:
  "I confirm I have the consent of the speaker whose voice this clones,
   or it is my own voice."
- `examples/cli/crispasr_run.cpp` — if voice ends in `.wav` and consent not
  set, error out with clear message; log consent with timestamp when set
- `examples/cli/crispasr_server.cpp` — if `voice` ends in `.wav` and
  `consent_attestation` field missing/empty in JSON body, return 400;
  log `[CONSENT] ts=<ISO8601> voice=<name> attestation="<text>"` to stderr

**Behavior:**
- CLI: `crispasr --tts "hello" --voice speaker.wav --i-have-rights` works;
  without `--i-have-rights` → error + explanation
- Server: `{"input":"hello","voice":"speaker.wav","consent_attestation":"I have consent"}` works;
  without attestation → 400

### Phase 3: Spoken disclaimer for voice-cloned output

**What:** Prepend "This audio was generated by artificial intelligence" to
voice-cloned TTS output using a neutral default voice (not the cloned voice).

**Approach:** Runtime synthesis via the loaded TTS backend with no voice ref
(speaker_id=0 / empty voice). Synthesize disclaimer text once on first
clone request; cache the PCM. Prepend to every voice-cloned output with
a 300ms silence gap.

**Files:**
- `examples/cli/crispasr_tts_disclaimer.h` — new header:
  `crispasr_tts_get_disclaimer(CrispasrBackend*, whisper_params&)` returns
  cached float32 PCM of the disclaimer. Thread-safe (std::call_once).
- `examples/cli/crispasr_server.cpp` — prepend disclaimer before watermark
  when voice is `.wav` (clone)
- `examples/cli/crispasr_run.cpp` — same for CLI

**Edge cases:**
- Backend doesn't support voiceless synthesis → use a short baked fallback
  (8-bit ADPCM C array, ~30 KB for 2.5s at 24 kHz)
- Disclaimer is NOT watermarked separately (the full output including
  disclaimer goes through the watermark)

### Phase 4: C2PA signed provenance metadata (self-signed)

**What:** Embed a C2PA manifest in WAV output declaring AI-generated
provenance, signed with a self-signed X.509 certificate.

**Approach:** Compile-time gated on `CRISPASR_HAVE_C2PA`. Uses the `c2pa-c`
library (C bindings for the Rust c2pa crate, MIT-licensed, pre-built
binaries: github.com/contentauth/c2pa-c).

**Files:**
- `cmake/Findc2pa.cmake` — CMake find module for c2pa-c (check for
  `c2pa.h` header and `libc2pa` shared lib)
- `examples/cli/crispasr_c2pa.h` — header wrapping c2pa-c API:
  - `crispasr_c2pa_sign_wav(std::string& wav, cert_path, key_path)` —
    creates manifest, signs it, embeds as C2PA RIFF chunk
  - `crispasr_c2pa_verify_wav(const std::string& wav)` — verify+print
- `examples/cli/crispasr_server.cpp` — call after WAV assembly
- `examples/cli/crispasr_run.cpp` — same for CLI
- `examples/cli/cli.cpp` — `--c2pa-cert` / `--c2pa-key` flags
- `scripts/generate-c2pa-cert.sh` — one-liner:
  `openssl req -x509 -newkey ec -pkeyopt ec_paramgen_curve:P-256 \
    -keyout crispasr-c2pa.key -out crispasr-c2pa.crt -days 3650 \
    -nodes -subj '/CN=CrispASR TTS/O=Self-Signed'`

**Manifest:**
```json
{
  "claim_generator": "CrispASR/0.6",
  "assertions": [{
    "label": "c2pa.actions",
    "data": { "actions": [{
      "action": "c2pa.created",
...
**Fallback:** When `CRISPASR_HAVE_C2PA` is not defined, skip silently.
The WAV LIST/INFO + watermark still provide provenance. Print a one-time
warning at server startup: "C2PA signing disabled (c2pa-c not found)".

### Phase 5: AudioSeal port to ggml/GGUF (future, separate branch)

**What:** Replace the spread-spectrum watermark with Meta's AudioSeal for
stronger robustness against adversarial removal / lossy compression.

**Approach:**
1. Export AudioSeal generator (SEANet encoder-decoder, ~5M params) from
   PyTorch → ONNX → GGUF (using gguf-py, same as codec models)
2. Implement AudioSeal forward pass in ggml:
   - Conv1d stack (SEANet encoder) → watermark embedding → Conv1d stack (decoder)
   - Similar architecture to existing SNAC/Mimi/EnCodec codecs
3. Load as companion model: `--watermark-model audioseal-generator.gguf`
4. Detector: same GGUF conversion for `audioseal-detector.gguf`, exposed
   via `crispasr_watermark_detect()` C API (replaces spread-spectrum detect)
5. Fallback: when no AudioSeal GGUF is loaded, keep current spread-spectrum

**This is a separate branch/PR.** The current spread-spectrum watermark
ships as the default in v0.6; AudioSeal is the v0.7 upgrade path.
